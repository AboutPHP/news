 ...
 * @return array
 */
function uc_user_usercode($username) {
	$return =  call_user_func(UC_API_FUNC, 'user', 'makeusercode', array('username'=>$username));
	return UC_CONNECT == 'mysql' ? $return : uc_unserialize($return);
}
function uc_user_register($username, $password,$regip) {
	$return = call_user_func(UC_API_FUNC, 'user', 'register', array('username'=>$username, 'password'=>$password, 'ip'=>$regip));
	return UC_CONNECT == 'mysql' ? $return : uc_unserialize($return);
}
...
function uc_get_cityinfo(){
	$return = call_user_func(UC_API_FUNC, 'common', 'get_ChineseDistrict');
	return UC_CONNECT == 'mysql' ? $return : uc_unserialize($return);
}
